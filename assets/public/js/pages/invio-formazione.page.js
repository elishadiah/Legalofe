...
parasails.registerPage('invio-formazione', {
  ...
  mounted: function mounted() {
    $.get('/player?IDSquadra=' + window.SAILS_LOCALS.teamId + '&sort=["Ruolo ASC","Nome ASC"]', function (players) {
...
var totaleNumeroMassimoRiserve = 8;
var numeroMassimoRiserve = new Array(0, 1, -1, -1, -1);
var panchinaOrdinata = 0;
var regolaRigoristi = 0;
var moduliAmmessi = [[0, 1, 3, 4, 3], [0, 1, 3, 5, 2], [0, 1, 3, 6, 1], [0, 1, 4, 3, 3], [0, 1, 4, 4, 2], [0, 1, 4, 5, 1], [0, 1, 5, 2, 3], [0, 1, 5, 3, 2], [0, 1, 5, 4, 1]];
...
for (var i = 1; i <= 11 + totaleNumeroMassimoRiserve; arrFormazione[i] = arrRigoristi[i++] = -1) {
  ;
}
...
function ClickGiocatoreRosa(idG) {
  ...
  if (!gg.Formazione) {
    ...
    if (titolariInseriti < 11) {
      result = InserisciTitolare(gg, idG);
    } else {
      result = InserisciRiserva(gg, idG);
    }
    if (result) {
    ...
    if (gg.Formazione <= 11) {
      RimuoviTitolare(gg, idG);
    } else {
      RimuoviRiserva(gg, idG);
    }
...
function InserisciTitolare(gg, idG) {
  ...
  for (var m = 0; m < moduliAmmessi.length; m++) {
    if (moduloInserito[1] <= moduliAmmessi[m][1] && moduloInserito[2] <= moduliAmmessi[m][2] && moduloInserito[3] <= moduliAmmessi[m][3] && moduloInserito[4] <= moduliAmmessi[m][4]) {
      compatibile = true;
      break;
    }
  }
  ...
  for (var _i = titolariInseriti; _i >= p; _i--) {
    arrFormazione[_i + 1] = arrFormazione[_i];
    arrInvioFormazione[arrFormazione[_i]].Formazione = _i + 1;
  }
...
function InserisciRiserva(gg, idG) {
  ...
  if (numeroMassimoRiserve[gg.Ruolo] !== -1 && riserveInserite[gg.Ruolo] >= numeroMassimoRiserve[gg.Ruolo]) {
...
function InviaFormazione() {
  if (!ControllaFormazione()) {
    return;
  }

  var standing = GeneraFormazioneJson();

  var destinatari = [];
  document.querySelectorAll('[name="destinatari"]').forEach(function (el) {
    if (el.checked) {
...
function ControllaFormazione() {
  var inTrasferta = window.SAILS_LOCALS.teamId == window.SAILS_LOCALS.match.teamAway.id;
  ...
  if (inTrasferta) {
    var ok = moduloInserito[2] >= 4 && moduloInserito[4] <= 2;
    if (!ok) {
  ...
  if (totaleRiserveInserite < totaleNumeroMassimoRiserve) {
    if (!confirm('La formazione è incompleta, vuoi inviarla comunque?')) {
      return;
    }
  }
...
function GeneraFormazioneJson() {
  ...
  for (var i = 0; i < arrInvioFormazione.length; i++) {
    var player = arrInvioFormazione[i];
    var pos = player.Formazione;

    if (!player.Formazione) {
      pos = 100;
    }
    players.push({ id: player.id, pos: pos });
  }

  players = _.sortBy(players, ['pos']);

  return { players: players.map(function (p) {
      return p.id;
