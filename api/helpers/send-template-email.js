module.exports = {
  ...
  fn: async function(inputs) {
    ...
    if (dontActuallySend) {
      ...
      if (!sails.config.custom.nodemailerHost ||
          !sails.config.custom.nodemailerUser ||
          !sails.config.custom.nodemailerPass) {
        sails.log.error(`nodemailer creds: host=${!!sails.config.custom.nodemailerHost}, user=${!!sails.config.custom.nodemailerUser}, pass=${!!sails.config.custom.nodemailerPass}`)
        throw new Error(
          'Cannot deliver email to "'+inputs.to+'" because NodeMailer credentials are missing'
        )
      }

      // create reusable transporter object using the default SMTP transport
      let transporter = nodemailer.createTransport({
        host: sails.config.custom.nodemailerHost,
        port: 465,
        secure: true, // true for 465, false for other ports
        auth: {
          user: sails.config.custom.nodemailerUser,
          pass: sails.config.custom.nodemailerPass
        },
        tls: {
      ...
      transporter
        ...
        .then(info => { sails.log('Message sent', info) })
        .catch(err => { sails.log.error('Message not sent', err) })
    }//ﬁ
