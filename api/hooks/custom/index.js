        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (req.method === 'GET') {
              ...
              // Title fallback value to avoid crash
              if (!res.locals.title) {
                res.locals.title = ''
              }
            }//ﬁ

            // Populate current league
   
     
            if (req.param('season') || req.cookies.season) {
              req.league = await Lega
                .findOne({ season: req.param('season') || req.cookies.season })
            ...
            var loggedInUser = await User
              .findOne({
                id: req.session.userId
              })
              .populate('team')

            const leagueTeams = req.league.teams.map(t => t.id)
            loggedInUser.team = loggedInUser.team.find(t => leagueTeams.includes(t.id))
